...
import com.bariq.bariqstoryapp2.*
import com.bariq.bariqstoryapp2.data.Result
import com.bariq.bariqstoryapp2.databinding.ActivityAddStoryBinding
...
class AddStoryActivity : AppCompatActivity() {
    ...
    private fun startTakePhoto() {
        ...
        createTempFile(application).also {
            val photoURI: Uri = FileProvider.getUriForFile(
                ...
                "com.bariq.bariqstoryapp2.ui.story",
                it
            )
    ...
    private fun uploadImage() {
        if (getFile != null) {
            val etDescText = binding.etDesc.text.toString()
            if (etDescText.isEmpty()) {
                binding.etDesc.error = resources.getString(R.string.input_message, "Description")
            } else {
                showLoading(true)
                ...
                storyViewModel.addStory(token, imageMultipart, description, lat, lon).observe(this) { result ->
                    if (result != null) {
                        when(result) {
                            is Result.Loading -> {
                                showLoading(true)
